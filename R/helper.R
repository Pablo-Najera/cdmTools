...
cdmTools.tetrachoric2 <- function(dat, method = "Bo", delta = 0.007, maxit = 1e+06, cor.smooth = TRUE, progress = TRUE){
  dat <- as.matrix(dat)
  if(method == "Ol"){
    res <- cdmTools.polychoric2(dat = dat, cor.smooth = cor.smooth, maxiter = 100)
  }
  if(method != "Ol"){
    ...
    h1 <- t((dat == 1)) %*% (dat == 0)
    dfr$f11 <- matrix(t((dat == 1)) %*% (dat == 1), ncol = 1, byrow = TRUE) + 0.5
    dfr$f10 <- matrix(t((dat == 1)) %*% (dat == 0), ncol = 1, byrow = TRUE) + 0.5
    dfr$f01 <- matrix(t((dat == 0)) %*% (dat == 1), ncol = 1, byrow = TRUE) + 0.5
    dfr$f00 <- matrix(t((dat == 0)) %*% (dat == 0), ncol = 1, byrow = TRUE) + 0.5
    ...
    if(cor.smooth){TC <- sirt_import_psych_cor.smooth(x = TC)}
